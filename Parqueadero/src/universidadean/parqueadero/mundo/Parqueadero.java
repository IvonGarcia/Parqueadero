...
public class Parqueadero {
    ...
    public String metodo1() {
        int maximoTiempo = 0;
        String placaCMasTiempo = "";
        Puesto[] puestosC = this.puestos;
        int puestos1c = puestosC.length;

        for(int i = 0; i < puestos1c; ++i) {
            Puesto puesto = puestosC[i];
            Carro carro = puesto.darCarro();
            if (carro != null) {
                int tiempoParqueadero = carro.darTiempoEnParqueadero(this.horaActual);
                if (tiempoParqueadero > maximoTiempo) {
                    maximoTiempo = tiempoParqueadero;
                    placaCMasTiempo = carro.darPlaca();
                }
            }
        }

        return placaCMasTiempo;
    }

    public String metodo2() {
        int puestosdisponibles = this.calcularPuestosLibres();
        String estadoActual;
        if (puestosdisponibles == 0) {
            estadoActual = "El parqueadero esta lleno";
        } else {
            estadoActual = "Hay puestos disponibles";
        }
